...
To reduce BP variability from sequential measurements, all three modalities (Ksens-BP, Oscillo-BP, and Auscl-BP) were integrated into a single experimental assembly. This integrated measurement approach has been previously reported [@yang_performance_2026].

```{r fig.cap="\\textbf{Schematic block diagram of the integrated experimental system.} (1) Controller, (2) BP cuff with embedded K-Sensor, (3) oscillometric BP device, (4) stethoscope, (5) trigger device, and (6) computer. All signals were routed through the controller to the host computer for real-time processing and storage, enabling simultaneous Ksens-BP, Oscillo-BP, and Auscl-BP acquisition under identical cuff-pressure conditions.", fig.scap="Schematic block diagram of the integrated experimental system."}
...
```{r fig.cap="\\textbf{Setup of the K-Sensor BP monitor.} During cuff deflation, the K-Sensor continuously captures arterial wall vibrations associated with Korotkoff sounds. The signal is transmitted to a pretrained host-computer model to determine Ksens-BP.", fig.scap="Setup of the K-Sensor BP monitor."}
...
![**Schematic diagram of the Ksens-BP determination process.** Panel (a): Representative recording of cuff pressure during inflation/deflation and the simultaneously acquired K-Sensor waveform. Panel (b): Automated discrimination of Korotkoff-related pulses within the K-Sensor signal during cuff deflation. Panel (c): SBP and DBP are derived by mapping cuff pressure at the first and last detected Korotkoff pulses, respectively.](../outputs/figures/External/2-3c.png){fig-scap="Schematic diagram of the Ksens-BP determination process."}
...
Oscillo-BP error was analyzed at attempt and patient levels using descriptive comparisons — including standardized mean difference (SMD) to quantify covariate imbalance — and logistic regression models (mixed-effects logistic model for attempt-level data and standard logistic model for patient-level data). Odds ratios with 95% confidence intervals are reported. All tests were two-sided, and p < 0.05 was considered statistically significant.
