...
if (knitr::is_latex_output()) {
  table36_str <- as.character(as_latex(table36_gt))
  table36_str <- gsub("\\\\begin\\{table\\}.*?\\n", "", table36_str)
  table36_str <- gsub("\\\\end\\{table\\}", "", table36_str)
  table36_str <- gsub("\\\\caption\\*\\{.*?\\}", "", table36_str)
  table36_str <- gsub(
    "\\\\fontsize\\{[0-9.]+pt\\}\\{[0-9.]+pt\\}\\\\selectfont",
  ...
  cat(
    "\\captionsetup{type=table,skip=2pt}\n",
    "\\captionof{table}{Omron error descriptive summary.}\n",
    table36_str,
...
if (knitr::is_latex_output()) {
  emit_gt_captionof(
    table38_gt,
    "Mixed-effects logistic regression for Omron error."
  )
} else {
  table38_gt
}
