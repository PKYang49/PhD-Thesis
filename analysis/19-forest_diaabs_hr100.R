# analysis/19-forest_diaabs_hr100.R
# Forest plot for DIA absolute-error subgroup analysis with HR split:
#   HR <100 vs >=100

library(dplyr)
library(stringr)
library(ggplot2)
library(patchwork)
library(readr)

means_path <- "outputs/tables/subgroup_absdiff_dia_means_HR100.csv"
cons_path  <- "outputs/tables/subgroup_absdiff_dia_contrasts_HR100.csv"

means <- read_csv(means_path, show_col_types = FALSE)
cons  <- read_csv(cons_path, show_col_types = FALSE)

means <- means %>%
  mutate(
    subgroup = case_when(
      subgroup %in% c("HTN", "DM", "ASCVD_any", "other_ascvd", "PAD", "Af") ~ subgroup,
      subgroup %in% c("HR", "HR_binary", "HR_tertiles", "HR_clinical") ~ "HR",
      TRUE ~ subgroup
    )
  )

cons <- cons %>%
  mutate(
    subgroup = case_when(
      subgroup %in% c("HTN", "DM", "ASCVD_any", "other_ascvd", "PAD", "Af") ~ subgroup,
      subgroup %in% c("HR", "HR_binary", "HR_tertiles", "HR_clinical") ~ "HR",
      TRUE ~ subgroup
    )
  )

keep_sg <- c("HTN", "DM", "other_ascvd", "PAD", "Af", "HR")
means <- means %>% filter(subgroup %in% keep_sg)
cons  <- cons %>% filter(subgroup %in% keep_sg)

p_int <- cons %>%
  group_by(subgroup) %>%
  summarise(p_int = min(p, na.rm = TRUE), .groups = "drop")

subgroup_print <- function(sg) {
  sg <- as.character(sg)
  dplyr::if_else(sg %in% c("ASCVD_any", "other_ascvd"), "ASCVD", sg)
}

level_print <- function(sg, level) {
  level <- as.character(level)
  if (sg %in% c("HTN", "DM", "ASCVD_any", "other_ascvd", "PAD", "Af")) {
    if (level %in% c("Yes", "Y", "1", 1, TRUE)) return(paste0("with ", subgroup_print(sg)))
    if (level %in% c("No", "N", "0", 0, FALSE)) return(paste0("without ", subgroup_print(sg)))
  }
  if (sg == "HR") {
    if (level %in% c("<100", "< 100")) return("HR < 100")
    if (level %in% c(">=100", ">= 100")) return("HR >= 100")
    return(paste0("HR ", level))
  }
  paste0(subgroup_print(sg), ": ", level)
}

df <- means %>%
  mutate(
    subgroup2 = subgroup_print(subgroup),
    level_disp = mapply(level_print, subgroup, level, SIMPLIFY = TRUE)
  ) %>%
  left_join(p_int, by = "subgroup")

hr_parent <- df %>%
  filter(subgroup2 == "HR") %>%
  slice(1) %>%
  mutate(
    level = NA,
    level_disp = "HR",
    estimate = NA_real_,
    lower = NA_real_,
    upper = NA_real_
  )

df <- bind_rows(df, hr_parent) %>%
  mutate(
    subgroup2 = factor(subgroup2, levels = c("HTN", "DM", "ASCVD", "PAD", "Af", "HR")),
    level_rank = case_when(
      subgroup2 == "HR" & level_disp == "HR" ~ 0L,
      str_detect(level_disp, "^with") ~ 1L,
      str_detect(level_disp, "^without") ~ 2L,
      level_disp == "HR < 100" ~ 1L,
      level_disp == "HR >= 100" ~ 2L,
      TRUE ~ 99L
    )
  ) %>%
  arrange(subgroup2, level_rank) %>%
  mutate(
    row_id = row_number(),
    y = rev(row_id)
  ) %>%
  group_by(subgroup2) %>%
  mutate(show_p = row_number() == n()) %>%
  ungroup() %>%
  mutate(
    level_disp_plot = case_when(
      as.character(subgroup2) == "HR" & level_disp == "HR" ~ "HR",
      as.character(subgroup2) == "HR" ~ paste0("   ", str_remove(level_disp, "^HR\\s*")),
      TRUE ~ level_disp
    ),
    p_txt = ifelse(show_p,
      ifelse(is.na(p_int), "", ifelse(p_int < 0.001, "< 0.001", sprintf("%.3f", p_int))),
      ""
    ),
    ci_txt = ifelse(
      is.na(estimate), "",
      paste0(
        sprintf("%.2f", estimate), " (",
        sprintf("%.2f", lower), ", ",
        sprintf("%.2f", upper), ")"
      )
    )
  )

header <- tibble(
  y = max(df$y) + 1.65,
  ci_txt = "Mean Differences\n(95% CI)",
  p_txt = "p for interaction"
)

x_lim <- c(-10, 10)

left_tbl <- ggplot() +
  geom_text(data = df, aes(x = 0, y = y, label = level_disp_plot), hjust = 0, size = 8) +
  geom_text(data = df, aes(x = 0.8, y = y, label = ci_txt), hjust = 0, size = 8) +
  geom_text(data = header, aes(x = 1.1, y = y, label = ci_txt), hjust = 0.5, fontface = "bold", size = 7) +
  coord_cartesian(xlim = c(-0.05, 1.6), ylim = c(0.5, max(df$y) + 2.4), clip = "off") +
  theme_void() +
  theme(plot.margin = margin(10, 0, 10, 10))

df_forest <- df %>% filter(!is.na(estimate), !is.na(lower), !is.na(upper))

forest <- ggplot(df_forest, aes(x = estimate, y = y)) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  geom_errorbar(aes(xmin = lower, xmax = upper), width = 0.0, orientation = "y") +
  geom_point(size = 2.6) +
  scale_x_continuous(breaks = seq(-10, 10, by = 2)) +
  labs(title = "Forest plot", x = NULL, y = NULL) +
  annotate("text", x = -9.5, y = max(df$y) + 1.65, label = "Favor Oscillo-DBP", size = 8, hjust = 0) +
  annotate("text", x = 9.5, y = max(df$y) + 1.65, label = "Favor Ksens-DBP", size = 8, hjust = 1) +
  coord_cartesian(xlim = x_lim, ylim = c(0.5, max(df$y) + 2.4), clip = "off") +
  theme_minimal(base_size = 16) +
  theme(
    axis.text.y = element_blank(),
    axis.ticks.y = element_blank(),
    panel.grid.major.y = element_blank(),
    panel.grid.minor = element_blank(),
    plot.title = element_text(hjust = 0.5, size = 26),
    plot.margin = margin(10, 0, 10, 0)
  )

right_p <- ggplot() +
  geom_text(data = df, aes(x = 1, y = y, label = p_txt), hjust = 1, size = 8) +
  geom_text(data = header, aes(x = 1, y = y, label = p_txt), hjust = 1, fontface = "bold", size = 8) +
  coord_cartesian(xlim = c(0, 1), ylim = c(0.5, max(df$y) + 2.4), clip = "off") +
  theme_void() +
  theme(plot.margin = margin(10, 10, 10, 0))

p <- left_tbl + forest + right_p + plot_layout(widths = c(1.40, 1.75, 0.60))

dir.create("outputs/figures", showWarnings = FALSE, recursive = TRUE)
ggsave("outputs/figures/forest_DIA_absdiff_subgroups_HR100.png", p, width = 16, height = 8, dpi = 300)

p
