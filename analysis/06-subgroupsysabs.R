# analysis/11-subgroup_absdiff_sys.R
# Outcome (SYS):
#   Y = abs(K-SYS - C-SYS) - abs(O-SYS - C-SYS)
# Interpretation:
#   Y < 0 => K has smaller absolute error than O (better)
#   Y > 0 => K has larger absolute error than O
#
# Subgroups: HTN, DM, other_ascvd, Af, HR tertiles + HR continuous
# Mixed model (random intercept per ID):
#   Y ~ subgroup + (adjusters) + (1|ID)
# Then use emmeans to estimate subgroup-specific mean(Y) and (Yes-No) contrasts.
...
df_m <- df_long %>%
  mutate(
    ...
    # numeric BP
    `O-SYS` = suppressWarnings(as.numeric(as.character(`O-SYS`))),
    `K-SYS` = suppressWarnings(as.numeric(as.character(`K-SYS`))),
    `C-SYS` = suppressWarnings(as.numeric(as.character(`C-SYS`))),

    # outcome: difference of absolute errors
    Y_absdiff_sys = abs(`K-SYS` - `C-SYS`) - abs(`O-SYS` - `C-SYS`),
...
run_subgroup_abs <- function(data, subgroup_var, tag) {
  # adjusters (exclude subgroup itself; exclude HR_c if subgroup is HR_group)
  ...
  need <- unique(c("ID","Y_absdiff_sys", subgroup_var, adjusters))
  d <- data %>% drop_na(all_of(need))
  d[[subgroup_var]] <- as.factor(d[[subgroup_var]])

  form <- as.formula(
    paste0("Y_absdiff_sys ~ ", subgroup_var, " + ", paste(adjusters, collapse = " + "), " + (1|ID)")
  )
  m <- lmer(form, data = d, REML = TRUE)

  # subgroup-specific mean(Y) with 95% CI
  ...
  # contrasts: if binary Yes/No -> compute Yes - No
  # For 3-level HR_group -> compute T2-T1 and T3-T1
  con_df <- NULL
  lv <- levels(d[[subgroup_var]])
  if (length(lv) >= 2) {
    con <- contrast(emm, method = "trt.vs.ctrl", ref = 1) # compare to first level
...
for (nm in names(subgroups)) {
  v <- subgroups[[nm]]
  out <- run_subgroup_abs(df_m, v, nm)
...
# Here we test whether HR relates to Y_absdiff_sys (not an interaction; outcome already K vs O).
d_hr <- df_m %>%
  drop_na(ID, Y_absdiff_sys, HR_c, Age, Sex, arm, Af, HTN, DM, other_ascvd, PAD)

m_hr <- lmer(
  Y_absdiff_sys ~ HR_c + Age + Sex + arm + Af + HTN + DM + other_ascvd + PAD + (1|ID),
  data = d_hr, REML = TRUE
)

cf_hr <- as.data.frame(summary(m_hr)$coefficients)
cf_hr$term <- rownames(cf_hr); rownames(cf_hr) <- NULL
hr_row <- cf_hr %>%
  filter(term == "HR_c") %>%
  transmute(
...
write.csv(means_all, file.path(out_tables_dir, "subgroup_absdiff_sys_means.csv"),
          row.names = FALSE, fileEncoding = "UTF-8")
write.csv(cons_all, file.path(out_tables_dir, "subgroup_absdiff_sys_contrasts.csv"),
          row.names = FALSE, fileEncoding = "UTF-8")
write.csv(hr_row, file.path(out_tables_dir, "subgroup_absdiff_sys_HRcontinuous.csv"),
          row.names = FALSE, fileEncoding = "UTF-8")

sink(file.path(out_models_dir, "subgroup_absdiff_sys_HRcontinuous_summary.txt"))
print(summary(m_hr))
sink()

message("Saved subgroup means: outputs/tables/subgroup_absdiff_sys_means.csv")
message("Saved subgroup contrasts: outputs/tables/subgroup_absdiff_sys_contrasts.csv")
message("Saved HR continuous row: outputs/tables/subgroup_absdiff_sys_HRcontinuous.csv")
message("Saved HR continuous summary: outputs/models/subgroup_absdiff_sys_HRcontinuous_summary.txt")
