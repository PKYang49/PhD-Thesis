# analysis/12-subgroupsysdiff.R
# Outcome (SYS):
#   Y_diff_sys = (O-SYS - C-SYS) - (K-SYS - C-SYS)
# Interpretation:
...
run_subgroup_sys <- function(data, subgroup_var, tag) {
  ...
  need <- unique(c("ID", "Y_diff_sys", subgroup_var, adjusters))
  d <- data %>% drop_na(all_of(need))
  d[[subgroup_var]] <- as.factor(d[[subgroup_var]])

  form <- as.formula(
    paste0("Y_diff_sys ~ ", subgroup_var, " + ", paste(adjusters, collapse = " + "), " + (1|ID)")
  )
...
for (nm in names(subgroups)) {
  v <- subgroups[[nm]]
  out <- run_subgroup_sys(df_m, v, nm)
...
d_hr <- df_m %>%
  drop_na(ID, Y_diff_sys, HR_c, Age, Sex, arm, Af, HTN, DM, other_ascvd, PAD)

m_hr <- lmer(
  Y_diff_sys ~ HR_c + Age + Sex + arm + Af + HTN + DM + other_ascvd + PAD + (1|ID),
  data = d_hr, REML = TRUE
)
...
write.csv(means_all, file.path(out_tables_dir, "subgroup_diff_sys_means.csv"),
  row.names = FALSE, fileEncoding = "UTF-8")
write.csv(cons_all, file.path(out_tables_dir, "subgroup_diff_sys_contrasts.csv"),
  row.names = FALSE, fileEncoding = "UTF-8")
write.csv(hr_row, file.path(out_tables_dir, "subgroup_diff_sys_HRcontinuous.csv"),
  row.names = FALSE, fileEncoding = "UTF-8")

sink(file.path(out_models_dir, "subgroup_diff_sys_HRcontinuous_summary.txt"))
print(summary(m_hr))
sink()

message("Saved subgroup means: outputs/tables/subgroup_diff_sys_means.csv")
message("Saved subgroup contrasts: outputs/tables/subgroup_diff_sys_contrasts.csv")
message("Saved HR continuous row: outputs/tables/subgroup_diff_sys_HRcontinuous.csv")
message("Saved HR continuous summary: outputs/models/subgroup_diff_sys_HRcontinuous_summary.txt")
