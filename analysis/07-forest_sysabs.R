...
forest <- ggplot(df_forest, aes(x = estimate, y = y)) +
  ...
  annotate("text",
           x = -4.8, y = max(df$y) + 1.65,
           label = "Favor Oscillo-SBP", size = 8, hjust = 0) +
  annotate("text",
           x =  4.6, y = max(df$y) + 1.65,
           label = "Favor Ksens-SBP", size = 8, hjust = 1) +
...
ggsave("outputs/figures/3-4forest_SYS_absdiff_subgroups.png", p, width = 16, height = 8, dpi = 300)
