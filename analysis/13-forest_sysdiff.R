# analysis/13-forest_sysdiff.R
# Forest plot for SYS difference outcome:
#   Y_diff_sys = (O-SYS - C-SYS) - (K-SYS - C-SYS)
# Interpretation:
#   Y > 0 favors Ksens-SBP
#   Y < 0 favors Oscillo-SBP
#
# Inputs:
#   outputs/tables/subgroup_diff_sys_means.csv
#   outputs/tables/subgroup_diff_sys_contrasts.csv
...
means_path <- "outputs/tables/subgroup_diff_sys_means.csv"
cons_path  <- "outputs/tables/subgroup_diff_sys_contrasts.csv"

means <- read_csv(means_path, show_col_types = FALSE)
cons  <- read_csv(cons_path,  show_col_types = FALSE)

# ------------------------------------------------------------
# 1) Normalize subgroup names
# ------------------------------------------------------------
means <- means %>%
  mutate(
    subgroup = case_when(
...
# ------------------------------------------------------------
left_tbl <- ggplot() +
  geom_text(data = df, aes(x = 0,    y = y, label = level_disp), hjust = 0, size = 6) +
...
# 9) Forest panel
# ------------------------------------------------------------
forest <- ggplot(df, aes(x = estimate, y = y)) +
...
ggsave("outputs/figures/forest_SYS_diff_OC_minus_KC_subgroups.png", p, width = 16, height = 8, dpi = 300)
