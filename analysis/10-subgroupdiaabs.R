...
run_subgroup_abs_dia <- function(data, subgroup_var, tag) {
  ...
  form <- as.formula(
    paste0("Y_absdiff_dia ~ ", subgroup_var, " + ", paste(adjusters, collapse = " + "), " + (1|ID)")
  )
...
for (nm in names(subgroups)) {
  v <- subgroups[[nm]]
  out <- run_subgroup_abs_dia(df_m, v, nm)
...
d_hr <- df_m %>%
  drop_na(ID, Y_absdiff_dia, HR_c, Age, Sex, arm, Af, HTN, DM, other_ascvd, PAD)

m_hr <- lmer(
  Y_absdiff_dia ~ HR_c + Age + Sex + arm + Af + HTN + DM + other_ascvd + PAD + (1|ID),
  data = d_hr, REML = TRUE
)
...
write.csv(means_all, file.path(out_tables_dir, "subgroup_absdiff_dia_means.csv"),
  row.names = FALSE, fileEncoding = "UTF-8")
write.csv(cons_all, file.path(out_tables_dir, "subgroup_absdiff_dia_contrasts.csv"),
  row.names = FALSE, fileEncoding = "UTF-8")
write.csv(hr_row, file.path(out_tables_dir, "subgroup_absdiff_dia_HRcontinuous.csv"),
  row.names = FALSE, fileEncoding = "UTF-8")

sink(file.path(out_models_dir, "subgroup_absdiff_dia_HRcontinuous_summary.txt"))
print(summary(m_hr))
sink()

message("Saved subgroup means: outputs/tables/subgroup_absdiff_dia_means.csv")
message("Saved subgroup contrasts: outputs/tables/subgroup_absdiff_dia_contrasts.csv")
message("Saved HR continuous row: outputs/tables/subgroup_absdiff_dia_HRcontinuous.csv")
message("Saved HR continuous summary: outputs/models/subgroup_absdiff_dia_HRcontinuous_summary.txt")
